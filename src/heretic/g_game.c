...
static int G_CRL_NextGameSpeed (int direction)
{
    ...
    if (direction > 0)
    {
        ...
        {
            if (crl_game_speed < game_speed_steps[i])
            {
...
void G_CRL_ChangeGameSpeed (int direction, boolean show_message)
{
    ...
    if (netgame)
    {
        CT_SetMessage(&players[consoleplayer], CRL_GAME_SPEED_NA_N, false, NULL);
        return;
    }
...
//
    ...
    if (gamekeydown[key_crl_mlook] || gamekeydown[key_crl_mlook2])
    {
        crl_mouselook ^= 1;
        if (!crl_mouselook)
        {
        ...
        S_StartSound(NULL, sfx_switch);
        gamekeydown[key_crl_mlook] = gamekeydown[key_crl_mlook2] = false;
    }
...
//
    ...
    */
    ...
    {
        if ((gamekeydown[key_speed] || gamekeydown[key_speed2]) && !noartiskip)
        {
...
//
    ...
    if (gamekeydown[key_use] || gamekeydown[key_use2] || mouse_buttons(mousebuse, mousebuse2) || joybuttons[joybuse])
    {
        cmd->buttons |= BT_USE;
    ...
    if (singleplayer)
    {
        ...
        if ((gamekeydown[key_crl_cameramoveto] || gamekeydown[key_crl_cameramoveto2])
        && crl_spectating)
        {
        ...
        {
            CT_SetMessage(&players[consoleplayer], "MOVE TO SSG MAX", false, NULL);
            CRL_MoveTo_SSG_MAX();
        }
        ...
        if (gamekeydown[key_crl_clear_ssg_max] || gamekeydown[key_crl_clear_ssg_max2])
        {
            ...
            CT_SetMessage(&players[consoleplayer], "CLEARED SSG MAX", false, NULL);
        }

        // SEG - Move to MAX.
        if (gamekeydown[key_crl_moveto_seg_max] || gamekeydown[key_crl_moveto_seg_max2])
        {
            CT_SetMessage(&players[consoleplayer], "MOVE TO SEG MAX", false, NULL);
            CRL_MoveTo_SEG_MAX();
        }
        ...
        if (gamekeydown[key_crl_clear_seg_max] || gamekeydown[key_crl_clear_seg_max2])
        {
            ...
            CT_SetMessage(&players[consoleplayer], "CLEARED SEG MAX", false, NULL);
        }

        // OPN - Move to MAX.
        if (gamekeydown[key_crl_moveto_opn_max] || gamekeydown[key_crl_moveto_opn_max2])
        {
            CT_SetMessage(&players[consoleplayer], "MOVE TO OPN MAX", false, NULL);
            CRL_MoveTo_OPN_MAX();
        }
        ...
        if (gamekeydown[key_crl_clear_opn_max] || gamekeydown[key_crl_clear_opn_max2])
        {
            ...
            CT_SetMessage(&players[consoleplayer], "CLEARED OPN MAX", false, NULL);
        }
        ...
        if (gamekeydown[key_crl_clear_all_max] || gamekeydown[key_crl_clear_all_max2])
        {
            ...
            CRL_Get_Render_MAX(&CRL_MAX_opn);
            CRL_Get_Render_MAX(&CRL_MAX_pln);
            CT_SetMessage(&players[consoleplayer], "CLEARED ALL MAX VALUES", false, NULL);
        }
    }
...
boolean G_Responder(event_t * ev)
{
    ...
    && (ev->data1 == key_crl_demospeed || ev->data1 == key_crl_demospeed2)
    ...
    if (ev->type == ev_keyup
    && (ev->data1 == key_useartifact || ev->data1 == key_useartifact2))
    ...
    if (gamestate == GS_LEVEL && ev->type == ev_keydown
        && (ev->data1 == key_spy || ev->data1 == key_spy2)
        && !deathmatch)
    ...
    if (ev->type == ev_keydown
    && (ev->data1 == key_prevweapon || ev->data1 == key_prevweapon2))
    {
        next_weapon = -1;
    }
    else if (ev->type == ev_keydown
    && (ev->data1 == key_nextweapon || ev->data1 == key_nextweapon2))
    {
        next_weapon = 1;
    }
    ...
    switch (ev->type)
    {
        case ev_keydown:
            ...
            {
                crl_sneaking ^= 1;
                CT_SetMessage(plr, crl_sneaking ?
...
void G_DoSaveGame(void)
{
    ...
    if (vanilla_savegame_limit && vanilla_savegame_print_size)
    {
        // [JN] CRL - report how full the save is against the vanilla vanilla_savegame_max_size_hr limit.
        ...
        M_snprintf(savemsg, sizeof(savemsg),
                   "%s\r(%i / %i bytes)",
                   DEH_String(TXT_GAMESAVED), savegamelength, vanilla_savegame_max_size_hr);
        CT_SetMessage(&players[consoleplayer], savemsg, true, NULL);

        // Also print to console. Red on overflow, yellow otherwise.
        CRL_printf(savemsg, savegamelength > vanilla_savegame_max_size_hr);
    }
    else
    {
