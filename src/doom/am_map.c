...
static void AM_HighlightByTag (void)
{
    ...
    CRL_SetMessage (plr, hlmsg, false, NULL);
...
boolean AM_Responder (const event_t *ev)
{
    ...
    else if (ev->type == ev_mouse && !menuactive)
    {
        ...
        if (!followplayer && crl_automap_mouse_pan && (ev->data2 || ev->data3))
        {
            ...
            // per-event >> 5. Rotate-mode conversion into map axes happens
            // at consumption, in AM_MousePanning().
            mouse_pan_sub_x += (int64_t) ev->data2 * mouseSensitivity;
            mouse_pan_sub_y += (int64_t) ev->data3 * mouseSensitivity;
    ...
    else if (ev->type == ev_keydown)
    {
        ...
        else if (key == key_map_grid || key == key_map_grid2)
        {
            ...
            CRL_SetMessage(plr, DEH_String(crl_automap_grid ?
        ...
        else if (key == key_crl_map_overlay || key == key_crl_map_overlay2)
        {
            ...
            if (crl_automap_overlay)
            {
                CRL_SetMessage(plr, DEH_String(CRL_AUTOMAPOVERLAY_ON), false, NULL);
            }
            else
            {
                CRL_SetMessage(plr, DEH_String(CRL_AUTOMAPOVERLAY_OFF), false, NULL);
                // [JN] Redraw status bar background.
                st_fullupdate = true;
            }
        }
        else if (key == key_crl_map_mousepan || key == key_crl_map_mousepan2)
        {
            ...
            CRL_SetMessage(plr, DEH_String(crl_automap_mouse_pan ?
        ...
        else if (key == key_crl_map_teleport || key == key_crl_map_teleport2)
        {
            ...
            if (demorecording)
            {
                CRL_SetMessage(plr, CRL_MAPTELEPORT_NA_R, false, NULL);
            }
            
            if (followplayer)
            {
                // [JN] No cursor in follow mode, so inform that teleport is not possible.
                CRL_SetMessage(plr, CRL_MAPTELEPORT_NA_F, false, NULL);
            }
            else
            {
...
static void AM_drawGrid (void)
{
    ...
    // second (stamped in P_BlockLines/ThingsIterator). Uses the grid's own
    ...
    if (crl_automap_blocks && blocktouch && bmapwidth > 0 && bmapheight > 0)
    {
        ...
        // [JN] Rescan the stamps once per tic, not once per frame.
...
static void AM_drawCrosshair (void)
{
    ...
    I_VideoBuffer[(f_y + (f_h >> 1)) * SCREENWIDTH + (f_x + (f_w >> 1))] = XHAIRCOLORS; // single point for now
}
